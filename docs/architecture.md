...
| 0.3 | 2026-07-07 | Real semantic memory search working end-to-end: `memory_entries` table (pgvector, 384-dim) populated via local `all-MiniLM-L6-v2` embedding model (sentence-transformers) — no external API cost for embeddings. Verified query "education background" correctly retrieved a stored memory with no literal keyword overlap, confirming true semantic (not keyword) search. pgAdmin set up for direct, spreadsheet-style data editing/correction (data edits only — schema changes still go through Alembic exclusively). Planned (not yet built): simple browser-based form for frictionless daily memory entry, to support ongoing "keep the AI up to date about me" use. Operational lessons: (1) each Python service needs its own venv activated explicitly per terminal session — `cd` alone does not switch venvs, a wrong-venv install caused a confusing missing-module error; (2) low disk space (C: drive) can silently break large pip installs (e.g. torch) — keep an eye on free space, consider moving the project to a drive with more room; (3) `run.ps1` helper scripts added per service to reduce repetitive activate+run steps. |
| 0.4 | 2026-07-07 | Product named **Aime**. Full frontend shell built (Next.js, TypeScript, Tailwind): landing page (product-framed, not personal — reusable for future users), login/signup page (Google OAuth placeholder + email/username/password with client-side strength validation, real auth wiring pending backend), chat interface (persona switcher and text/voice-face toggle, both currently visual-only placeholders — no backend access-control yet), and admin panel (sections for Facts, Memories, Persona & Behavior, Character & Voice; Facts/Memories currently show realistic mock data in the same shape the real API returns, ready to swap for live calls). All API access centralized through `apps/web/src/lib/api.ts` — components never call `fetch` directly, so backend changes require editing only this one file. Fixed a CORS gap (ai-orchestrator lacked `CORSMiddleware`, blocking browser calls) and a context-completeness gap (system prompt exposed computed age but not the underlying DOB, causing inconsistent answers to differently-phrased questions — fixed by exposing both). Verified temporal fact/memory accuracy end-to-end via real chat: correct age, correct current graduate status (superseding an earlier "still studying" memory), correct arrival-in-Germany timeline — all through the live browser chat UI, not just API testing. Deferred to later: real authentication/authorization, persona-based data access enforcement, voice/face rendering, admin CRUD wired to live endpoints. |
